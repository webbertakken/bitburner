import {configure, window} from './utils/index.js'
...
export async function main(ns) {
  ...
  while (true) {
    ...
    ns.print(`hack security effect: ${ns.hackAnalyzeSecurity(1, node.id)} times 10: ${ns.hackAnalyzeSecurity(10, node.id)}`)
    ns.print('\n')
    ns.print(`grow time: ${ns.tFormat(ns.getGrowTime(node.id))}`)
    ns.print(`grow effect: ${ns.growthAnalyze(node.id, 1.1, 1)}`)
    ns.print(`grow security effect: ${ns.growthAnalyzeSecurity(1, node.id)} times 10: ${ns.growthAnalyzeSecurity(10, node.id)} times${Math.round(ns.growthAnalyze(node.id, 1.1, 1))}: ${ns.growthAnalyzeSecurity(Math.round(ns.growthAnalyze(node.id, 1.1, 1)), node.id)}`)
    ns.print('\n')
